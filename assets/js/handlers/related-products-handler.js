...
(function ($) {
  ...
  var RelatedProductsHandler = {
    ...
    bindEvents: function () {
      ...
      $(document).on(
        "click",
        ".snap-sidebar-cart__add-related-product",
        function (e) {
          e.preventDefault();
    ...
    updateSliderNavigation: function ($track) {
      var childrenCount = $track.children(
      ...
      if (childrenCount > 0) {
        ...
        var $firstItem = $track
          .children(".snap-sidebar-cart__related-product")
          .first();
    ...
    changeTab: function (tabType) {
      ...
      $('.snap-sidebar-cart__related-tab[data-tab="' + tabType + '"]').addClass(
        "active"
      );
      ...
      var $targetContainer = $targetContent.find(
    ...
    loadRelatedProducts: function (tabType) {
      ...
      if ($targetContainer.length === 0) {
        console.error(
          "Contenedor no encontrado para productos de tipo:",
          tabType
        );
        return;
      }
    ...
    setupProductGalleryHover: function () {
      $(".snap-sidebar-cart__related-product").each(function () {
        ...
        var $imageContainer = $product.find(
          ".snap-sidebar-cart__related-product-image"
        );
        ...
        if ($galleryImages.length) {
          ...
          if ($galleryImages.length > 1) {
            $product.on("mousemove", function (e) {
              ...
              var imageIndex = Math.min(
                Math.floor(ratio * imageCount),
                imageCount - 1
              );
    ...
    addRelatedProductToCart: function (productId, $button) {
      ...
      $(".snap-sidebar-cart__product").each(function () {
        if ($(this).data("product-id") == productId) {
          ...
          existingProductQuantity =
            parseInt($(this).find(".cart-item__quantity-input").val(), 10) || 0;
      ...
      if (existingProduct && existingProductKey) {
        ...
        var $existingItem = $(
          '.snap-sidebar-cart__product[data-key="' + existingProductKey + '"]'
        );

        // Mostrar preloader
        if ($existingItem.length) {
          var $loader = $existingItem.find(
        ...
        // Añadir preloader
        var $preloader = $(
          '<div class="snap-sidebar-cart__product-loader" style="display:block;"><div class="snap-sidebar-cart__loader-spinner preloader-circle"></div></div>'
        );
