...
(function ($) {
  ...
  $(document).ready(function () {
    console.log(
      "Inicializando navegación mejorada para slider de productos relacionados"
    );
    ...
    function updateSliderNavigation($track) {
      ...
      if (currentScrollLeft >= maxScrollLeft - 5) {
        // 5px de margen para evitar problemas de redondeo
    ...
    function checkCarouselVisibility($track) {
      ...
      if (totalItemsWidth <= containerWidth) {
        console.log(
          "Los productos caben en el contenedor, ocultando navegación"
        );
        $prevButton.hide();
        $nextButton.hide();
      } else {
    ...
    $(document)
      .off("click.sliderPrev", ".snap-sidebar-cart__slider-prev")
      .on("click.sliderPrev", ".snap-sidebar-cart__slider-prev", function (e) {
        e.preventDefault();
        e.stopPropagation();
        console.log("Clic en botón anterior del slider");

        var $track = $(this)
          .closest(".snap-sidebar-cart__related-container.active")
          .find(".snap-sidebar-cart__slider-track");
        if (!$track.length) {
    ...
    $(document)
      .off("click.sliderNext", ".snap-sidebar-cart__slider-next")
      .on("click.sliderNext", ".snap-sidebar-cart__slider-next", function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $(document.body).on("snap_sidebar_cart_updated", function () {
      console.log(
        "Evento de actualización de carrito detectado - reinicializando sliders"
      );
      setTimeout(function () {
        initializeAllSliders();
      }, 300);
