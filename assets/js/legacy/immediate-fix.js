...
jQuery(function($) {
    ...
    $(window).on('load', function() {
        setTimeout(function() {
            // 0. Verificar y crear contenedores si no existen
            checkContainers();
    ...
    // Función para verificar y crear contenedores si no existen
    function checkContainers() {
        console.log('Verificando contenedores...');
        
        // Verificar contenedor principal
        var $sidebarCart = $('.snap-sidebar-cart');
        if (!$sidebarCart.length) {
            console.error('Contenedor principal del carrito no encontrado');
            return;
        }
        
        // Verificar contenedor de productos relacionados
        var $relatedSection = $sidebarCart.find('.snap-sidebar-cart__related-section');
        if (!$relatedSection.length) {
            console.log('Creando sección de productos relacionados');
            $relatedSection = $('<div class="snap-sidebar-cart__related-section"></div>');
            $sidebarCart.find('.snap-sidebar-cart__body').append($relatedSection);
        }
        
        // Verificar contenedor de pestañas
        var $tabsContainer = $relatedSection.find('.snap-sidebar-cart__related-tabs');
        if (!$tabsContainer.length) {
            console.log('Creando contenedor de pestañas');
            $tabsContainer = $('<div class="snap-sidebar-cart__related-tabs"></div>');
            $relatedSection.append($tabsContainer);
        }
        
        // Verificar contenedores de productos
        var tabTypes = ['upsells', 'crosssells'];
        tabTypes.forEach(function(type) {
            var $container = $('.snap-sidebar-cart__related-container[data-content="' + type + '"]');
            if (!$container.length) {
                console.log('Creando contenedor para', type);
                $container = $('<div class="snap-sidebar-cart__related-container" data-content="' + type + '"></div>');
                $relatedSection.append($container);
                
                // Crear estructura interna del contenedor
                var $sliderContainer = $('<div class="snap-sidebar-cart__related-slider-container"></div>');
                var $sliderWrapper = $('<div class="snap-sidebar-cart__related-slider-wrapper"></div>');
                var $slider = $('<div class="snap-sidebar-cart__related-slider"></div>');
                ...
                var $navigation = $('<div class="snap-sidebar-cart__related-navigation"></div>');
                var $prevBtn = $('<button class="snap-sidebar-cart__related-prev" aria-label="Anterior"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg></button>');
                var $nextBtn = $('<button class="snap-sidebar-cart__related-next" aria-label="Siguiente"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg></button>');
                
                $navigation.append($prevBtn).append($nextBtn);
                $sliderContainer.append($navigation);
            }
        });
        
        console.log('Verificación de contenedores completada');
    }
